...
<body class="bg-slate-100 min-h-screen text-slate-800 flex flex-col md:flex-row pb-20 md:pb-0" 
    ...
    <!-- BOTTOM NAVIGATION (Pengalaman Mobile Nyaman) -->
    <nav class="fixed bottom-0 left-0 right-0 bg-slate-900/95 backdrop-blur-md text-slate-400 border-t border-slate-800 z-40 flex justify-around items-center p-2 md:hidden">
        ...
        <a href="tambah_santri.php" class="flex flex-col items-center -mt-5">
    ...
    <main class="flex-1 flex flex-col min-w-0 overflow-x-hidden">
        ...
        <header class="bg-white/80 backdrop-blur-md border-b border-slate-200 px-4 sm:px-6 py-4 flex items-center justify-between sticky top-0 z-20">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h2 class="text-lg sm:text-xl font-bold text-slate-800">Data Santri</h2>
                    <p class="text-xs text-slate-500 hidden sm:block">Kelola seluruh data santri tahfidz</p>
                </div>
            </div>
            <a href="tambah_santri.php" class="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white rounded-xl text-xs font-semibold shadow-md shadow-emerald-600/20 transition-all flex items-center space-x-2">
        ...
        <div class="p-4 sm:p-6 space-y-5 animate-fade-in">
            ...
            <div class="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col sm:flex-row gap-3 justify-between items-center">
                ...
                <div class="text-xs text-slate-500 w-full sm:w-auto text-right">
                    Total Santri: <span class="font-bold text-slate-800"><?php echo mysqli_num_rows($result); ?></span>
                </div>
            </div>
                    ...
                    <table class="w-full text-left border-collapse text-xs sm:text-sm">
                        <thead>
                            <tr class="bg-slate-50/80 text-slate-500 text-[11px] uppercase tracking-wider border-b border-slate-100">
                                <th class="py-3.5 px-4 sm:px-5 font-semibold">NIS</th>
                                <th class="py-3.5 px-4 sm:px-5 font-semibold">Nama Santri</th>
                                <th class="py-3.5 px-4 sm:px-5 font-semibold">Kelas</th>
                                <th class="py-3.5 px-4 sm:px-5 font-semibold">Wali Santri</th>
                                <th class="py-3.5 px-4 sm:px-5 font-semibold">Capaian Hafalan</th>
                        ...
                        <tbody class="divide-y divide-slate-100 text-slate-700">
                            <?php if (mysqli_num_rows($result) > 0): ?>
                                <?php while ($row = mysqli_fetch_assoc($result)): 
                                    $searchableText = strtolower($row['nis'] . ' ' . $row['nama'] . ' ' . $row['kelas_kelompok'] . ' ' . ($row['nama_wali'] ?? ''));
                                ?>
                                    <tr class="hover:bg-slate-50/80 transition-colors"
                                        x-show="'<?php echo addslashes($searchableText); ?>'.includes(search.toLowerCase())">
                                        
                                        <!-- NIS -->
                                        <td class="py-3.5 px-4 sm:px-5 font-medium text-slate-600 whitespace-nowrap">
                                        ...
                                        <td class="py-3.5 px-4 sm:px-5 font-semibold text-slate-800 whitespace-nowrap">
                                            <div class="flex items-center space-x-3">
                                                <div class="w-8 h-8 rounded-full bg-emerald-100 text-emerald-700 font-bold flex items-center justify-center text-xs flex-shrink-0">
                                                    <?php echo strtoupper(substr($row['nama'], 0, 1)); ?>
                                                </div>
                                                <div>
                                                    <p class="leading-tight"><?php echo htmlspecialchars($row['nama']); ?></p>
                                                    <span class="text-[11px] font-normal text-slate-400 block"><?php echo htmlspecialchars($row['email']); ?></span>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="py-3.5 px-4 sm:px-5 whitespace-nowrap text-slate-600">
                                            <?php echo htmlspecialchars($row['kelas_kelompok']); ?>
                                        </td>

                                        <!-- WALI -->
                                        <td class="py-3.5 px-4 sm:px-5 whitespace-nowrap text-slate-600">
                                            <?php echo htmlspecialchars($row['nama_wali'] ?? '-'); ?>
                                        </td>
                                        ...
                                        <td class="py-3.5 px-4 sm:px-5 text-center whitespace-nowrap space-x-1 sm:space-x-2">
                                            ...
                                            <a href="santri.php?action=delete&id=<?php echo $row['id']; ?>" 
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="7" class="py-12 text-center text-slate-400 text-xs">
                                        ...
                                        Belum ada data santri. Silakan klik tombol <b class="text-slate-600">Tambah Santri</b> untuk menambah data.
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
