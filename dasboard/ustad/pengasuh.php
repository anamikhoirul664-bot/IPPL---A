...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    ...
    $get_user = mysqli_query($koneksi, "SELECT user_id FROM pengasuh WHERE id = $pengasuh_id");
    if ($data = mysqli_fetch_assoc($get_user)) {
        $user_id_del = $data['user_id'];
        
        // Lepas relasi pengasuh_id di tabel halaqah agar tidak error
        mysqli_query($koneksi, "UPDATE halaqah SET pengasuh_id = NULL WHERE pengasuh_id = $pengasuh_id");
        
        // Hapus akun di tabel users (cascade menghapus data di tabel pengasuh)
...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    ...
    if ($data = mysqli_fetch_assoc($get_user)) {
        ...
        // Lepas relasi ustad_id di tabel halaqah agar tidak error
        mysqli_query($koneksi, "UPDATE halaqah SET ustad_id = NULL WHERE ustad_id = $user_id_del");
...
<body class="bg-slate-100 min-h-screen text-slate-800 flex">
    ...
    <aside class="w-64 bg-slate-900 text-slate-300 flex flex-col min-h-screen sticky top-0 z-30">
        ...
        <nav class="flex-1 p-4 space-y-1 overflow-y-auto text-sm">
            ...
            <a href="pengasuh.php" class="flex items-center space-x-3 px-4 py-3 rounded-xl bg-emerald-600 text-white font-medium shadow-lg shadow-emerald-600/30">
    ...
    <main class="flex-1 flex flex-col min-w-0 overflow-x-hidden">
        ...
        <header class="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 z-20">
            <div>
                <h2 class="text-xl font-bold text-slate-800">Data Pengasuh / Ustadz</h2>
                <p class="text-xs text-slate-500">Kelola akun ustadz dan pembimbing halaqah</p>
            </div>
            <a href="tambah_pengasuh.php" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-medium shadow-md shadow-emerald-600/20 transition-all flex items-center space-x-2">
                    ...
                    <table class="w-full text-left border-collapse text-sm">
                        <thead>
                            <tr class="bg-slate-50 text-slate-500 text-xs uppercase tracking-wider border-b border-slate-100">
                                <th class="py-3 px-5 font-semibold">Nama Pengasuh</th>
                                <th class="py-3 px-5 font-semibold">NIP / Kode</th>
                                <th class="py-3 px-5 font-semibold">Kontak</th>
                                <th class="py-3 px-5 font-semibold">Spesialisasi</th>
                                <th class="py-3 px-5 font-semibold">Bimbingan</th>
                        ...
                        <tbody class="divide-y divide-slate-100 text-slate-700">
                                ...
                                <?php while ($row = mysqli_fetch_assoc($result)): ?>
                                    <tr class="hover:bg-slate-50/80 transition-colors">
                                        <td class="py-3.5 px-5 font-semibold text-slate-800">
                                            ...
                                            <span class="block text-xs font-normal text-slate-400">Username: <?php echo htmlspecialchars($row['username']); ?></span>
                                        </td>
                                        <td class="py-3.5 px-5 font-mono text-xs text-slate-600"><?php echo htmlspecialchars($row['nip'] ?? '-'); ?></td>
                                        <td class="py-3.5 px-5 text-xs text-slate-600">
                                            <div><i class="fa-solid fa-phone text-slate-400 mr-1"></i> <?php echo htmlspecialchars($row['telepon'] ?? '-'); ?></div>
                                            <div><i class="fa-solid fa-envelope text-slate-400 mr-1"></i> <?php echo htmlspecialchars($row['email']); ?></div>
                                        </td>
                                        <td class="py-3.5 px-5 text-slate-600"><?php echo htmlspecialchars($row['spesialisasi'] ?? 'Tahfizh Umum'); ?></td>
                                        <td class="py-3.5 px-5 text-xs">
                                            <span class="px-2 py-0.5 rounded bg-slate-100 text-slate-700 font-medium mr-1"><?php echo $row['total_halaqah']; ?> Halaqah</span>
                                            <span class="px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 font-medium"><?php echo $row['total_santri']; ?> Santri</span>
                                        </td>
                                        ...
                                        <td class="py-3.5 px-5 text-center space-x-2">
                                            <a href="edit_pengasuh.php?id=<?php echo $row['id']; ?>" class="px-2.5 py-1.5 bg-amber-50 text-amber-600 hover:bg-amber-100 rounded-lg text-xs font-medium transition-colors">
                                                <i class="fa-solid fa-pen-to-square"></i> Edit
                                            </a>
                                            <a href="pengasuh.php?action=delete&id=<?php echo $row['id']; ?>" onclick="return confirm('Apakah Anda yakin ingin menghapus data pengasuh ini?');" class="px-2.5 py-1.5 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg text-xs font-medium transition-colors">
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="7" class="py-8 text-center text-slate-400 text-xs">Belum ada data pengasuh. Silakan klik <b>Tambah Pengasuh</b>.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
