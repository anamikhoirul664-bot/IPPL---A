...
$stmt_curr = mysqli_prepare($koneksi, "SELECT p.*, u.nama, u.username, u.email, u.telepon, u.status 
                                       FROM pengasuh p 
                                       JOIN users u ON p.user_id = u.id 
                                       WHERE p.id = ?");
mysqli_stmt_bind_param($stmt_curr, "i", $pengasuh_id);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nama) || empty($username) || empty($email)) {
        ...
        // Validation check for username/email duplication (excluding current user)
    ...
    <main class="flex-1 flex flex-col min-w-0 overflow-x-hidden">
        ...
        <header class="bg-white/80 backdrop-blur-md border-b border-slate-200 px-4 sm:px-6 py-4 flex items-center justify-between sticky top-0 z-20">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h2 class="text-lg sm:text-xl font-bold text-slate-800">Edit Data Pengasuh</h2>
                    <p class="text-xs text-slate-500 hidden sm:block">Perbarui informasi akun dan detail pengasuh halaqah</p>
                </div>
            </div>
            
            <a href="pengasuh.php" class="px-3 sm:px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-semibold transition-colors flex items-center space-x-1 sm:space-x-2">
        ...
        <div class="p-4 sm:p-6 max-w-4xl w-full mx-auto space-y-6 animate-fade-in">
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 p-4 sm:p-6 shadow-sm">
                <form action="edit_pengasuh.php?id=<?php echo $pengasuh_id; ?>" method="POST" class="space-y-6">
                    ...
                    <div>
                        <div class="flex items-center space-x-2 pb-3 mb-4 border-b border-slate-100">
                            ...
                            <h3 class="text-xs sm:text-sm font-bold text-slate-800 uppercase tracking-wider">Data Kepegawaian & Detail</h3>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="spesialisasi" value="<?php echo htmlspecialchars($data['spesialisasi']); ?>" placeholder="misal: Tajwid / Mutqin" 
                                       class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all">
                            </div>
                    ...
                    <div class="pt-4 border-t border-slate-100 flex flex-col sm:flex-row justify-end items-center gap-3">
                        <a href="pengasuh.php" class="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-slate-100 text-slate-600 hover:bg-slate-200 text-xs sm:text-sm font-semibold transition-colors text-center">
                            Batal
                        </a>
