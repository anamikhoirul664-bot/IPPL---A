...
// Ambil data wali saat ini
$query_curr = "SELECT ws.*, u.nama, u.username, u.email, u.telepon, u.status 
               FROM wali_santri ws JOIN users u ON ws.user_id = u.id WHERE ws.id = $wali_id";
...
<body class="bg-slate-100 min-h-screen text-slate-800 flex">
    ...
    <main class="flex-1 p-6 max-w-4xl mx-auto">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-xl font-bold text-slate-800">Edit Data Wali Santri</h2>
                <p class="text-xs text-slate-500">Perbarui profil dan akun login wali santri</p>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-sm">
            <form action="edit_wali.php?id=<?php echo $wali_id; ?>" method="POST" class="space-y-5">
                ...
                <h3 class="text-sm font-bold text-emerald-600 uppercase tracking-wider border-b border-slate-100 pb-2 pt-4">Informasi Pribadi & Status</h3>
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="pekerjaan" value="<?php echo htmlspecialchars($data['pekerjaan']); ?>" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500">
                    </div>
                    <div>
                ...
                <div class="pt-4 flex justify-end space-x-3">
                    ...
                    <button type="submit" class="px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium transition-all shadow-md shadow-emerald-600/20">
                        <i class="fa-solid fa-save mr-1"></i> Perbarui Data Wali
                    </button>
                </div>
