...
$query_curr = "SELECT s.*, u.nama, u.username, u.email, u.telepon, u.status 
               FROM santri s JOIN users u ON s.user_id = u.id WHERE s.id = $santri_id";
...
<body class="bg-slate-100 min-h-screen text-slate-800 flex">
    ...
    <main class="flex-1 p-6 max-w-4xl mx-auto">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-xl font-bold text-slate-800">Edit Data Santri</h2>
                <p class="text-xs text-slate-500">Perbarui profil dan informasi akun santri</p>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-sm">
            <form action="edit_santri.php?id=<?php echo $santri_id; ?>" method="POST" class="space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="nis" value="<?php echo htmlspecialchars($data['nis']); ?>" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500">
                    </div>
                    <div>
                ...
                <h3 class="text-sm font-bold text-emerald-600 uppercase tracking-wider border-b border-slate-100 pb-2 pt-4">Informasi Akademik & Status</h3>
                <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">Kelas / Kelompok</label>
                        <input type="text" name="kelas_kelompok" value="<?php echo htmlspecialchars($data['kelas_kelompok']); ?>" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="number" name="target_juz" value="<?php echo $data['target_juz']; ?>" min="1" max="30" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500">
                    </div>
                    <div>
                ...
                <div class="pt-4 flex justify-end space-x-3">
                    ...
                    <button type="submit" class="px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium transition-all shadow-md shadow-emerald-600/20">
                        <i class="fa-solid fa-save mr-1"></i> Perbarui Data Santri
                    </button>
                </div>
