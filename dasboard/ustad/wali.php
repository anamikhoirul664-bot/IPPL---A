...
<body class="bg-slate-100 min-h-screen text-slate-800 flex">
    ...
    <main class="flex-1 flex flex-col min-w-0 overflow-x-hidden">
        ...
        <header class="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 z-20">
            <div>
                <h2 class="text-xl font-bold text-slate-800">Data Wali Santri</h2>
                <p class="text-xs text-slate-500">Kelola akun dan data orang tua / wali santri</p>
            </div>
            <a href="tambah_wali.php" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-medium shadow-md shadow-emerald-600/20 transition-all flex items-center space-x-2">
                    ...
                    <table class="w-full text-left border-collapse text-sm">
                        <thead>
                            <tr class="bg-slate-50 text-slate-500 text-xs uppercase tracking-wider border-b border-slate-100">
                                <th class="py-3 px-5 font-semibold">Nama Wali</th>
                                <th class="py-3 px-5 font-semibold">Kontak</th>
                                <th class="py-3 px-5 font-semibold">Pekerjaan</th>
                                <th class="py-3 px-5 font-semibold">Santri yang Diampu</th>
                        ...
                        <tbody class="divide-y divide-slate-100 text-slate-700">
                            <?php if (mysqli_num_rows($result) > 0): ?>
                                <?php while ($row = mysqli_fetch_assoc($result)): ?>
                                    <tr class="hover:bg-slate-50/80 transition-colors">
                                        ...
                                        <td class="py-3.5 px-5 text-slate-600"><?php echo htmlspecialchars($row['pekerjaan'] ?? '-'); ?></td>
                                        <td class="py-3.5 px-5">
                                            <?php if ($row['total_anak'] > 0): ?>
                                                <span class="font-medium text-emerald-700"><?php echo htmlspecialchars($row['daftar_santri']); ?></span>
                                                <span class="block text-xs text-slate-400">(<?php echo $row['total_anak']; ?> Santri)</span>
                                            <?php else: ?>
                                        ...
                                        <td class="py-3.5 px-5 text-center space-x-2">
                                            <a href="edit_wali.php?id=<?php echo $row['id']; ?>" class="px-2.5 py-1.5 bg-amber-50 text-amber-600 hover:bg-amber-100 rounded-lg text-xs font-medium transition-colors">
                                                <i class="fa-solid fa-pen-to-square"></i> Edit
                                            </a>
                                            <a href="wali.php?action=delete&id=<?php echo $row['id']; ?>" onclick="return confirm('Apakah Anda yakin ingin menghapus wali santri ini?');" class="px-2.5 py-1.5 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg text-xs font-medium transition-colors">
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="6" class="py-8 text-center text-slate-400 text-xs">Belum ada data wali santri. Silakan klik <b>Tambah Wali Santri</b>.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
