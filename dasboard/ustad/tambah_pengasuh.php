...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nama) || empty($username) || empty($email) || empty($password)) {
        ...
        // Cek username/email unik dengan Prepared Statement
    ...
    <main class="flex-1 flex flex-col min-w-0 overflow-x-hidden">
        ...
        <header class="bg-white border-b border-slate-200 px-4 md:px-6 py-4 sticky top-0 z-20 flex items-center justify-between">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h2 class="text-base md:text-xl font-bold text-slate-800 leading-tight">Tambah Pengasuh Baru</h2>
                    <p class="text-[10px] md:text-xs text-slate-500">Buat akun Ustadz / Ustadzah pengampu halaqah</p>
                </div>
            </div>
            <a href="pengasuh.php" class="px-3 py-1.5 md:px-4 md:py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-medium transition-colors flex items-center">
                ...
                <form action="" method="POST" class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="nama" required placeholder="Ust. Ahmad Fauzi, S.Pd.I" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs md:text-sm focus:outline-none focus:border-emerald-500">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1">Username *</label>
                            <input type="text" name="username" required placeholder="ust_fauzi" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs md:text-sm focus:outline-none focus:border-emerald-500">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1">Email *</label>
                            <input type="email" name="email" required placeholder="fauzi@pesantren.com" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs md:text-sm focus:outline-none focus:border-emerald-500">
                        </div>
                        <div>
                    ...
                    <h3 class="text-xs md:text-sm font-bold text-emerald-600 uppercase tracking-wider border-b border-slate-100 pb-2 pt-4">Data Kepegawaian & Kontak</h3>
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1">NIP / Kode Pengasuh</label>
                            <input type="text" name="nip" placeholder="UST-2026-001" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs md:text-sm focus:outline-none focus:border-emerald-500">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1">Nomor Telepon / WA</label>
                            <input type="text" name="telepon" placeholder="081298765432" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs md:text-sm focus:outline-none focus:border-emerald-500">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1">Spesialisasi / Keahlian</label>
                            <input type="text" name="spesialisasi" placeholder="Tahfizh / Tajwid / Mutqin" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs md:text-sm focus:outline-none focus:border-emerald-500">
                        </div>
                        <div class="sm:col-span-3">
                            <label class="block text-xs font-semibold text-slate-700 mb-1">Alamat Lengkap</label>
                            <textarea name="alamat" rows="2" placeholder="Komplek Pesantren..." class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs md:text-sm focus:outline-none focus:border-emerald-500"></textarea>
                        </div>
                    </div>

                    <div class="pt-4 flex items-center justify-end space-x-3">
                        <a href="pengasuh.php" class="px-5 py-2.5 rounded-xl bg-slate-100 text-slate-600 hover:bg-slate-200 text-xs font-medium transition-colors">Batal</a>
